...
- 状态：桌面 MVP 功能闭环已建立，`v0.1.7` Windows Pre-release 准备发布
- 最新 Windows Release 构建提交：即将由 `v0.1.7` 标签触发
- 最后更新：2026-06-18
- 当前版本：`v0.1.7` Windows Pre-release
...
当前阶段不是继续扩展功能范围，而是先补强对话后编码任务的执行质量，再使用真实 Windows 11
安装环境完成私测准入验收、修复阻断缺陷，并补齐正式发布前的安全和交付控制。

## 2. 最新权威发布

| 项目 | 结果 |
| --- | --- |
| Windows Release 构建提交 | `v0.1.7` 标签提交 |
| Windows Release | 待 GitHub Actions 生成 |
| GitHub Pre-release | [Mimodex v0.1.7](https://github.com/MorganArthur/mimodex/releases/tag/v0.1.7) |
| 安装包 | `Mimodex_0.1.7_x64-setup.exe`，未签名 NSIS x64 |
| 安装包大小 | 待 Release 生成后回填 |
| SHA256 | 待 Release 生成后回填 |
...
- 文本、工具调用和用量流式事件；
- 简单对话快速路径与增量绘制；
- `v0.1.7` 补强 Codex 风格任务执行指令：先读项目、遵守 `AGENTS.md`、保护用户改动、聚焦修改并验证；
- `v0.1.7` 调整 MiMo thinking 策略：无工具简单对话继续禁用 thinking，带工具编码任务不再强制禁用 thinking；
