...
- 只读、工作区写入和完全访问模式展示；
- 任务提交、右侧用户气泡、左侧流式 AI 与工具活动、轮次处理时间和停止控制；
- 命令与文件审批卡片；
- 工作区文件变更摘要面板；
...
线程工作区按用户消息组织轮次：用户内容显示为右侧气泡，不显示“你”标签；AI 正文
显示在左侧，不显示“MiMo”标签；推理、命令、文件和状态显示为默认收起的紧凑活动行，
需要诊断时可展开查看原始投影内容。每个新轮次记录真实开始与完成时间，执行期间显示
“处理中”，完成后显示“已处理”及耗时；旧历史记录没有时间戳时不伪造耗时。
...
- Runtime 连接状态与平台信息；
- 当前项目、模型、线程、轮次和执行状态；
- 用户消息、助手文本、推理、命令、文件与错误时间线及条目开始、完成时间；
- 待处理审批及其决定；
- 当前轮次聚合 Diff。

桌面根服务同时将线程相关 Runtime 原始协议事件批量写入 SQLite。右侧“活动”面板
按线程读取最近 `300` 条持久化事件，并实时合并当前连接中新到达的事件；每条记录可
展开查看方向、方法、消息类型和原始 JSON，便于私测诊断与权限审计。
...
最近线程也不再由界面硬编码。桌面层保存 Runtime 线程 ID 与受限 UI 投影，用户选择
历史线程时调用 `thread/resume` 恢复权威上下文。该索引不是 Provider 历史的事实
来源，详细边界见 `THREAD_INDEX_PERSISTENCE.md`。
...
浏览器验收已覆盖默认模型、高级模型、项目添加与切换、设置诊断、任务提交、左右对话
布局、轮次处理时间、流式推理、命令审批、停止状态、完成状态、文件级变更摘要和活动
审计，浏览器控制台无错误或警告。
