...
- 状态：桌面 MVP 与持久化活动审计版 Windows 技术预览已通过权威构建
- 最后更新：2026-06-13
...
- 当前可用构建：[Windows Preview #27449796215](https://github.com/MorganArthur/mimodex/actions/runs/27449796215)
...
当前持久化活动审计版 Artifact：
[mimodex-windows-preview-6c09352efccd504173c178e81b5c152ff60de754](https://github.com/MorganArthur/mimodex/actions/runs/27449796215/artifacts/7605689013)。
该构建增加按线程查询最近 `300` 条 Runtime 原始协议事件、实时活动合并、历史线程
活动恢复和可展开原始 JSON 的右侧活动面板。它通过 Desktop CI、Tauri Rust 格式与
编译检查、SQLite 持久化 Rust 单测、Runtime sidecar 编译与初始化握手、NSIS 打包和
SHA256 校验。安装包大小为 `57.76 MiB`，SHA256 为
`DE138717BECCAFBA419DB768FA7C62C07632BA1292E8F6725BB5F0B66E8579A4`。

## 4. 当前使用限制

- 当前安装包仍是未签名内部技术预览，Windows SmartScreen 可能显示警告；
- 应用内凭据、真实项目、权限边界、旧 JSON 迁移、归档与活动恢复仍需完成真实安装验收；
- 已提供连接诊断、自定义 API Base URL 和结构化错误引导，但限流、超时、上下文超限
  与服务不可用仍需真实 Provider 失败验证；
- 正式代码签名、安装/卸载自动化冒烟和 GitHub Release 工作流仍未提供；
- CI 会执行 Runtime `initialize` 握手，但正式发布前仍需执行真实 Windows 11 安装、
  启动、Agent 闭环和卸载验收。

## 5. 下一阶段

1. 按 `docs/validation/WINDOWS_11_PRIVATE_BETA_ACCEPTANCE.md` 完成真实 Windows 11
   安装、凭据、权限、Git、线程恢复和活动审计验收；
2. 修复验收发现的阻断缺陷，并补齐安全脱敏、异常恢复和动作与变更关联测试；
3. 为签名发布工作流补齐安装、启动、Runtime 握手和卸载冒烟测试。

最新构建和剩余工作统一见：[Mimodex 当前项目状态](../CURRENT_STATUS.md)。
