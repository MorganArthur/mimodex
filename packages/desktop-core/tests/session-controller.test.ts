...
test("上下文窗口接近阈值时下一轮自动注入压缩指令", async () => {
  ...
  const turnInput = runtime.turnStarts.at(-1)?.input[0];
  const submitted = turnInput?.type === "text" ? turnInput.text : "";
...
test("恢复历史线程会等待正在进行的 Runtime 初始化", async () => {
  const runtime = new FakeRuntimeClient();
  const barrier = deferred<void>();
  runtime.initializeBarrier = barrier.promise;
  const session = new DesktopSessionController(runtime);

  const connect = session.connect();
  assert.equal(session.getSnapshot().connection, "connecting");
  const resume = session.resumeThread({
    threadId: "thread-history",
    projectPath: "D:\\project",
    model: "mimo-v2.5",
  ...
  assert.equal(session.getSnapshot().threadId, "thread-history");
  assert.equal(runtime.threadResumes.length, 0);
  barrier.resolve();
  await connect;
  await resume;

  assert.equal(runtime.threadResumes.length, 1);
  assert.equal(runtime.threadResumes[0]?.threadId, "thread-history");
  assert.equal(session.getSnapshot().connection, "ready");
});
...
test("启动任务时将附带的图片记入用户时间线并作为多模态输入发送给 Runtime", async () => {
  const runtime = new FakeRuntimeClient();
  const session = new DesktopSessionController(runtime);
  ...
  assert.deepEqual(runtime.turnStarts[0]?.input, [
    { type: "image", url: "data:image/png;base64,AAAA" },
    { type: "text", text: "看一下这张截图", textElements: [] },
  ]);
});
